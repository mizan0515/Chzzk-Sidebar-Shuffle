...
function Find-WhaleExecutable {
  $running = Get-CimInstance Win32_Process |
    Where-Object { $_.Name -ieq 'whale.exe' -and $_.CommandLine -match '^\s*"([^"]+whale\.exe)"' } |
    Select-Object -First 1

  if ($running -and $running.CommandLine -match '^\s*"([^"]+whale\.exe)"') {
    return $Matches[1]
  }
...
Start-Process -FilePath $whaleExe -ArgumentList $args | Out-Null
