...
assert.match(popupHtml, /html\s*\{[\s\S]*width:\s*390px;[\s\S]*min-width:\s*360px;/, 'Extension popup root should reserve a real width before Chrome sizes the action popup');
assert.match(popupHtml, /body\s*\{[\s\S]*width:\s*390px;[\s\S]*min-width:\s*360px;/, 'Extension popup body must not depend on 100vw, which can collapse to a narrow action popup');
assert.doesNotMatch(popupHtml, /width:\s*min\(390px,\s*100vw\)/, 'Popup width must not use a viewport-dependent min() that can collapse in Chrome action popup sizing');
